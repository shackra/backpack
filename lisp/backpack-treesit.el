...
(defun backpack-treesit-was-asked (gear &optional minimal-emacs-version-expected)
  ...
  (let* ((asked                         (eval `(gearp! :editing ,gear)))
	 (disabled-globally-tree-sitter (gearp! :ui -treesit))
	 (disabled-at-gear              (eval `(gearp! :editing ,gear -treesit)))
	...
	(progn
	  (when (and asked
		     (not disabled-globally-tree-sitter)
		     (not disabled-at-gear)
		     (not version-ok))
	    (lwarn 'backpack :warning  "cannot enable tree-sitter support for gear %s, minimal Emacs version wanted %s but got %s" gear minimal-emacs-version-expected emacs-version))
